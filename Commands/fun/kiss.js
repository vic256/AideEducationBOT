  ...
  execute(message, args, bot, embedfooter) {
    let muser = message.mentions.users.first()
        ...
        if (!message.mentions.users.first()) return message.channel.sendMessage({
             "embed": {
                     description: "**" + message.author.username + "**" + ", vous avez reçu un baiser de " +  `** ${config.botname} !**`,
                     color: 0xff7b00,
                     "image": {
                     "url": image,
                     timestamp: new Date(),
        ...
        message.channel.sendMessage({
             "embed": {
                     description: "** " + muser.username + "**"  + ", vous avez reçu un baiser de " + "**" + message.author.username + " **",
                     color: 0xff7b00,
                     "image": {
                     "url": image,
                     timestamp: new Date(),
